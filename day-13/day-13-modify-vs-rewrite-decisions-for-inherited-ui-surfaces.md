...
The analysis frameworks were introduced at orientation depth on Day 1 — see [[day-01-brownfield-decision-frameworks]]. You've also applied a subset of them in narrower contexts: [[day-08-modeling-polymorphic-data]] used technical-debt and ROI-horizon analysis for a Pydantic modeling call; [[day-10-multi-service-feature-delivery]] applied impact mapping to a cross-cutting infrastructure change; [[day-11-api-contract-design-under-inherited-ui-constraints]] used SWOT and cost-benefit for a backend contract decision. Today combines the full toolkit on a single decision.
...
2. **Propagates auth context end-to-end.** Whatever auth model the inherited surface uses (cookie session, JWT in header, NextAuth, etc.), it must reach the API calls. A request without auth context fails the criterion regardless of how cleanly the UI renders. See [[day-13-authentication-context-propagation-through-the-component-tree]] for the propagation patterns.
3. **Persists state across navigation.** A learner moving from question 2 to question 3 and back to question 2 must see their previously-selected answer still selected. State must outlive the route transition. See [[day-13-multi-step-ui-navigation-without-state-loss]].
...
5. **Renders whichever question shapes were finalized on Day 8.** The polymorphic-modeling decision from Day 8 — discriminated union, tagged enum, or per-type subclasses — produces a question payload of a specific shape. The UI must render every shape that backend will emit. See [[day-13-polymorphic-component-rendering-for-variant-data-types]].
...
- **Treating "modify" as the default brownfield-friendly answer.** The brownfield mindset of [[day-01-brownfield-mindset---reading-inherited-code-finding-the-seams]] is "read first, change second" — not "always modify." Rewrite is sometimes correct; the curriculum supports it explicitly. What's wrong is rewriting *without* analysis.
...
---
*Prerequisites: [[day-01-brownfield-decision-frameworks]], [[day-01-brownfield-mindset---reading-inherited-code-finding-the-seams]], [[day-08-modeling-polymorphic-data]], [[day-10-multi-service-feature-delivery]], [[day-11-api-contract-design-under-inherited-ui-constraints]]. Connects within Day 13 to [[day-13-dynamic-routing-with-app-router-parameters]], [[day-13-server-components-for-initial-data-fetching]], [[day-13-client-components-for-stateful-interactivity]], [[day-13-type-safe-api-integration]], [[day-13-authentication-context-propagation-through-the-component-tree]], [[day-13-polymorphic-component-rendering-for-variant-data-types]], and [[day-13-multi-step-ui-navigation-without-state-loss]].*
