...
- **`server_now`** lets the client compute an offset for the countdown display without trusting its own clock. The deeper rationale lives in `day-11-server-authoritative-state`.
...
- **Computing `expires_at` on the client and trusting it.** See `day-11-server-authoritative-state`.
...
---
*Prerequisites: day-8 question-authoring backend topics, day-11-opaque-token-generation-and-session-identifiers.*
