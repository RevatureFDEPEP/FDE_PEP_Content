...
Before you touch a single line of code in the inherited `rev-eval-ai` repository, you need to understand what role you are stepping into and where this four-week cohort sits in Revature's larger talent-development pipeline. The "Forward Deployed Engineer" (FDE) role at Revature is defined by *ownership of outcomes*, not by a single tier of the stack, and the Pre-Employment Programme (PEP) is deliberately shaped to build that ownership mindset before you are placed on a client engagement. This topic frames the *why* behind everything else you will do this week — and explains why an engineering programme spends its first week debugging Docker Compose and CI pipelines instead of writing product features.

## The FDE Role at Revature

At Revature, a "Forward Deployed Engineer" is defined by what they own: the *outcome* the client needs, not a single layer of the stack. The job is to be dropped into a client's environment and make the thing work — wherever the work happens to be. Concretely, you are expected to:

- **Own the outcome, end-to-end.** When a problem lands on your plate, you are accountable for the whole path to a working result — the UI, the API call that feeds it, the contract with the backend service, and the operational story (does it work in staging? are the logs useful?). You are not handed a narrow ticket and told to do only the part inside your comfort zone; you are handed an outcome and expected to drive it home.
- **Work fluently across the stack to deliver.** You are not expected to be a database expert or a platform engineer, but you *are* expected to read a Python service, change a small thing safely, debug a Docker Compose stack-up, and reason about a CI pipeline. The bar is "competent collaborator across every tier the outcome touches," not "specialist in one of them."
...
- **Take ownership of the operational surface.** When something breaks anywhere on the path to your outcome — local environment, CI, deployment — you investigate before escalating. You are not expected to fix every operational issue alone, but you are expected to be able to *describe* it accurately to whoever can.
...
The specific stack (Python+Next.js+Docker vs. Django+Vue+K8s) is incidental. The *pattern* — own the outcome in an unfamiliar system, wherever the work leads — is what transfers.

## Common Pitfalls

- **Mistaking "Forward Deployed Engineer" for a single-tier role.** Candidates who arrive expecting to live in one layer of the stack — only the UI, only the API — are surprised by Week 1's operational focus. Reset that expectation now — your job here is to own outcomes across whatever tiers they touch, not to specialize in one.
...
- "Forward Deployed Engineer" at Revature means an engineer who owns outcomes end-to-end and works fluently across whatever tiers those outcomes touch — not a specialist confined to a single layer of the stack.
