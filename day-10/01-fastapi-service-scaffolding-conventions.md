...
- An `alembic/` directory (for relational services — see [02-alembic-for-relational-schema-evolution.md](02-alembic-for-relational-schema-evolution.md)).
- A `tests/` directory with at least one passing test (proves the harness works).
- An entry in `compose.yml` (covered in [03-service-composition-patterns-in-docker-compose.md](03-service-composition-patterns-in-docker-compose.md)).
- An entry in the CI workflow matrix (covered in [04-extending-ci-workflows-for-new-tests-and-services.md](04-extending-ci-workflows-for-new-tests-and-services.md)).
...
Multi-stage rationale (covered in [02-multi-stage-dockerfiles.md](../day-02/02-multi-stage-dockerfiles.md)): keep build tools out of the runtime image. The healthcheck uses stdlib `urllib` so we don't add `curl` to the image.
...
---
*Prerequisites: [02-multi-stage-dockerfiles.md](../day-02/02-multi-stage-dockerfiles.md), day-8 backend slice topics.*
