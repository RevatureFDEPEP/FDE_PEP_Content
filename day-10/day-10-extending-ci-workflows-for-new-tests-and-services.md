...
That's the whole edit. Because every service follows the scaffolding convention from `day-10-fastapi-service-scaffolding-conventions`, the same steps work:
...
This job pairs nicely with the migration discipline from `day-10-alembic-for-relational-schema-evolution` — `alembic upgrade head` runs against a real Postgres on every CI run, catching broken migrations before merge.
...
The `cache-dependency-path` per-service means each service has its own cache key — adding a dep to user-service doesn't invalidate reporting's cache. See `day-3-build-caching-strategies-in-ci`.
...
---
*Prerequisites: day-3 GitHub Actions topics, day-7 mature CI topics, day-10-fastapi-service-scaffolding-conventions.*
